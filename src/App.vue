      ...
      <form @submit.prevent="handleSubmit" class="box mt-5">
        ...
        <div class="field">
          ...
          <div class="control has-icons-left" :class="{ 'is-loading': this.isLoading }">
            <input type="email" id="user-email" name="email" class="input" :placeholder="this.stage === Stage.isLogin ? 'Enter your email' : 'You need to register'
              " v-model="user.email" :disabled="this.stage === Stage.isRegister || this.isLoading" required />
        ...
        <div v-if="this.stage === Stage.isRegister" class="field">
          ...
          <div class="control has-icons-left">
            <input type="text" id="user-name" name="name" class="input" placeholder="Enter your name"
              v-model="user.name" :disabled="isLoading" required />
        ...
        <div class="tile is-ancesor">
          <div class="tile is-parent">
            <div class="tile is-child box is-success">
              <PostsTable :sidebar="this.sidebar" :isLoading="this.isLoading" :currentPost="this.currentPost"
                :posts="this.posts" @sidebarSetAddPost="this.sidebar = Sidebar.addPost" @openPost="openPost($event)" />
            </div>
          </div>

          <div v-if="sidebar !== Sidebar.isClosed" class="tile is-parent is-8-desktop Sidebar"
            :class="{ 'Sidebar--open': sidebar !== Sidebar.isClosed }">
            <div class="tile is-child box is-success">
              <div class="content">
                <div v-if="sidebar === Sidebar.addPost || this.sidebar === Sidebar.editPost" class="content">
                  <h2>
                    {{
                      this.sidebar === Sidebar.editPost
                        ? 'Post editing'
                  ...
                  <form @submit.prevent="submitAddPost">
                    <div class="field" data-cy="NameField">
                      ...
                      <div class="control has-icons-left has-icons-right">
                        <input type="text" name="title" id="comment-author-name-title" placeholder="Post title"
                          class="input" v-model="this.postTitle" required /><span class="icon is-small is-left"><i
                            class="fas fa-user fa-heading"></i></span>
                      </div>
                    </div>
                    <div class="field" data-cy="BodyField">
                      ...
                      <div class="control">
                        <textarea id="comment-body" name="body" placeholder="Post body" class="textarea"
                          v-model="this.postBody" required></textarea>
                      </div>
                    </div>
                ...
                <div v-if="sidebar === Sidebar.showPost || sidebar === Sidebar.addComment" class="content">
                  <div class="block">
                    <div class="is-flex is-justify-content-space-between is-align-items-center">
                      ...
                      <div class="is-flex">
                        ...
                        <span class="icon is-small is-right has-text-danger is-clickable ml-3" @click="deletePost"><i
                  ...
                  <div v-else-if="this.sidebar !== Sidebar.addComment" class="block">
                    <template v-if="this.comments.length > 0">
                      <article v-for="comment of this.comments" :key="comment.id" class="message is-small"
                        data-cy="Comment">
                        <div class="message-header">
                          <a href="mailto:dfg" data-cy="CommentAuthor">{{ comment.name }}</a>
                          <button data-cy="CommentDelete" type="button" class="delete is-small" aria-label="delete"
                    ...
                    <button data-cy="WriteCommentButton" type="button" class="button is-link" @click="createComment">
                      Write a comment
                    </button>
                  </div>

                  <AddCommentForm v-if="this.sidebar === Sidebar.addComment" v-model:email="this.newComment.email"
                    v-model:name="this.newComment.name" v-model:body="this.newComment.body"
                    :errorMessage="this.errorMessage" :sidebar="this.sidebar"
                    @sidebarSetShowPost="this.sidebar = Sidebar.showPost" @submitAddComment="submitAddComment()" />
                </div>
              </div>
            </div>
          </div>

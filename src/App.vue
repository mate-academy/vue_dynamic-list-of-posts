<template>
  <div>
    <section
      v-if="!user"
      class="container is-flex is-justify-content-center mt-5"
    >
      <form
        @submit.prevent="login"
        class="box"
        style="max-width: 275px; width: 100%"
      >
        <h1 class="title is-3" style="color: var(--color-heading)">
          Get your userId
        </h1>
        ...
        <div class="field">
          ...
          <div
            ...
            :class="{ 'has-icons-right': errors.loginEmail }"
          >
            <input
              v-model.trim="email"
              type="email"
          ...
          <p v-if="errors.loginEmail" class="help is-danger">
            A valid email is required
          </p>
        </div>

        <p v-if="loginError" class="has-text-danger mb-2">
          {{ loginError }}
        </p>
    ...
    <div v-else>
      <nav class="navbar is-light" role="navigation" style="background-color: white">
        <div class="navbar-brand">
          <h2
            class="is-size-4 ml-5 mt-3 has-text-weight-semibold"
        ...
        <div class="navbar-end mr-3">
          <div class="navbar-item">
            <div class="buttons">
              ...
              <button class="button is-light logout-btn" @click="logout">
                Logout
              </button>
            </div>
          </div>
      ...
      <div class="tile is-ancestor mt-6" style="max-width: 1400px; margin: 0 auto">
        <div class="tile is-parent is-4-desktop ml-5">
          ...
          >
              ...
              <tbody>
                <tr v-for="post in posts" :key="post.id">
                  ...
                  <td class="has-text-right">
                    ...
                    >
                      {{
                        selectedPost && selectedPost.id === post.id && sidebarOpen
                          ? 'Close'
                          : 'Open'
                      }}
                    </button>
            ...
            <p v-if="errorPosts" class="has-text-danger">{{ errorPosts }}</p>
            <p
        ...
        <Sidebar :open="sidebarOpen">
          <template v-if="selectedPost">
            ...
            <div v-else>
              <p v-if="comments.length === 0" class="block has-text-weight-semibold is-size-4">
                No comments yet
              </p>
              <CommentForm
          ...
          <template v-else-if="creatingPost">
            <h2 class="title is-4" style="color: var(--color-heading)">
              Create new post
            </h2>
            <form @submit.prevent="savePost">
              <div class="field">
                ...
                <div
                  class="control has-icons-left"
                  :class="{ 'has-icons-right': errors.newPostTitle }"
                >
                  <input
                    class="input"
                ...
                <p v-if="errors.newPostTitle" class="help is-danger">
                  Title is required
                </p>
              </div>
              <div class="field">
                ...
                <p v-if="errors.newPostBody" class="help is-danger">
                  Body is required
                </p>
              </div>
...
const validatePostFields = () => {
  ...
  if (!newPost.value.title) {
    errors.newPostTitle = true;
    isValid = false;
  }
  if (!newPost.value.body) {
    errors.newPostBody = true;
    isValid = false;
  }
  return isValid;
};
...
const validatePostField = (field) => {
  if (field === 'title') {
    errors.newPostTitle = !newPost.value.title;
  }
  if (field === 'body') {
    errors.newPostBody = !newPost.value.body;
  }
};
...
async function loadPosts() {
  ...
  try {
    ...
    errorPosts.value = 'Failed to load posts.';
  } finally {

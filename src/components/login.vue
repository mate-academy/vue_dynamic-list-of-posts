        ...
        <form class="box mt-5" @submit.prevent="register ? Register() : handleLogin(email)">
            ...
            <div class="field">
                ...
                <div class="control has-icons-left">
                    <input type="email" id="user-email" name="email" class="input" placeholder="Enter your email"
                        required v-model.trim="email" :disabled="register || loading" autocomplete="off" />
            ...
            <template v-if="register">
                <div className="field">
                    ...
                    <div className="control has-icons-left">
                        <input type="text" id="user-name" name="name" className="input" placeholder="Enter your name"
                            required minlength="{4}" v-model.trim="name" />
            ...
            <div class="field">
                <Loader v-if="loading" />
                <template v-else>
                    <button type="submit" class="button is-primary" :class="{ 'is-loading': loading }" v-if="register">
                        Register
                    </button>
                    <button type="submit" class="button is-primary" :class="{ 'is-loading': loading }" v-else>
                        Login
                    </button>
                </template>
            </div>

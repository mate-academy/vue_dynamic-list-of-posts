<template>
  <form @submit.prevent="handleSubmit">
    ...
    <div class="field" data-cy="NameField">
      <label class="label" :for="`comment-author-name-${postId}`">
        Author Name
      </label>
    ...
    <div class="field" data-cy="EmailField">
      <label class="label" :for="`comment-author-email-${postId}`">
        Author Email
      </label>
      <div class="control has-icons-left has-icons-right">
        <input
    ...
    <div class="field" data-cy="BodyField">
      <label class="label" :for="`comment-body-${postId}`">
        Comment Text
      </label>
      <div class="control">
        <textarea
...
export default {
  ...
  methods: {
    // Valida os campos do comentário (nome, email, formato do email e corpo)
    validate() {
      let isValid = true;
    ...
    // Processa o envio do formulário de novo comentário
    async handleSubmit() {
      this.serverError = '';
      ...
      try {
        ...
        this.$emit('created', comment); // Emite o novo comentário criado
        // Mantemos o nome e o email para facilitar múltiplos comentários, limpando apenas o texto
        this.body = '';
      } catch (error) {
    ...
    // Limpa apenas o campo de texto e os erros visuais
    handleClear() {
      this.body = '';
      this.errors = {
        name: '',
